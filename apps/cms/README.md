...
Launcher сам проверит зависимости, запустит PostgreSQL и MinIO, передаст
безопасные локальные значения в Strapi и дождётся `/_health`.

Полностью локальный сайт вместе с CMS:
...
`dev:full` автоматически создаёт временные local-only read/write API tokens:
значения остаются в памяти процессов, в PostgreSQL записываются только хэши.
Ручное копирование токенов в env-файлы не требуется.
...
Низкоуровневый `npm run develop` внутри `apps/cms` остаётся доступен для
специальных сценариев, но не является стандартным способом развёртывания.
