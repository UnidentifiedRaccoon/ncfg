    ...
    <Section id="season-offer-tasks" title={title} lead={lead}>
      <ol className="grid list-none gap-4 p-0 sm:grid-cols-2 lg:grid-cols-3 lg:gap-6">
        {tasks.map((task, index) => {
          ...
          return (
            <li key={task.id}>
              <Reveal variant="card" delay={motionTokens.stagger * (index % 3)} className="h-full">
                <article
                className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-[#E2E8F0] bg-white p-5 shadow-sm transition-[border-color,box-shadow,transform] duration-200 motion-safe:hover:-translate-y-1 hover:border-[#3B82F6]/35 hover:shadow-lg motion-reduce:transform-none motion-reduce:transition-none md:p-6"
              >
                ...
                <ProgramDetailsLink
                  ...
                  label={`${task.program.label}: перейти к описанию программы`}
                >
                  {task.program.label}
                </ProgramDetailsLink>
                </article>
