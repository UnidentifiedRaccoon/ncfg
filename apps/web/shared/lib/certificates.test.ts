...
test("sortCertificatesByYear orders newer certificates first and keeps empty years last", () => {
  const newest = transform(createMedia(), {
    documentId: "2025-b",
    year: 2025,
    order: 2,
  });
  const sameYearEarlier = transform(createMedia(), {
    documentId: "2025-a",
    year: 2025,
    order: 1,
  });
  const older = transform(createMedia(), {
    documentId: "2023-a",
    year: 2023,
    order: 0,
  });
  const withoutYear = transform(createMedia(), {
    documentId: "no-year",
    year: null,
    order: 0,
  });

  const sorted = sortCertificatesByYear([
    older,
    newest,
    withoutYear,
    sameYearEarlier,
  ...
  assert.deepEqual(
    sorted.map((certificate) => certificate.id),
    ["2025-a", "2025-b", "2023-a", "no-year"]
  );
});
